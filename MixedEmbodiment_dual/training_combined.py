"""
MixedEmbodiment_dual ACT training (Method A bridge).

- Mixed demos live in the human-side loader (concat); no separate mixed loader
  recycled every step. Batches stay single-embodiment via a homogeneous sampler.
- Mixed batch => TWO routes with correct masks:
    robot: joints + robot CVAE + mixed cam mask + joint head
    human: pose + human CVAE + bird-only cam mask
- --mixed_human_pose full8 (default): human route uses all 8 glued pose dims
- --mixed_human_pose hand4: human route keeps only the hand half
"""
...
from MixedEmbodiment_dual.build_sync import (  # noqa: E402
    _infer_preset,
    _resolve_eef_dir,
    _resolve_pose_dir,
    build_mixed_sync_csvs,
)
from MixedEmbodiment_dual.config import (  # noqa: E402
    DEFAULT_BATCH_SIZE,
    DEFAULT_HAND_LAMBDA,
    ...
    HUMAN_CAMERA_MASK,
    HUMAN_POSE_RELDIR,
    MIXED_HUMAN_POSE_FULL8,
    MIXED_HUMAN_POSE_MODES,
    MODEL_CAMERA_NAMES,
    NUM_CAMERAS,
    POSE_DIM,
    ROBOT_EEF_RELDIR,
    ROBOT_JOINT_DIM,
    apply_mixed_human_pose_mode,
    build_run_metadata,
    default_run_name,
...
from MixedEmbodiment_dual.core import build, kl_divergence  # noqa: E402
from MixedEmbodiment_dual.data_synchronization import (  # noqa: E402
    synchronize_human_hands,
    synchronize_robot_bimanual,
)
from MixedEmbodiment_dual.dataloader_human import HumanEpisodeDataset, collate_homogeneous  # noqa: E402
from MixedEmbodiment_dual.dataloader_mixed import (  # noqa: E402
    ConcatMixedEpisodeDataset,
    MixedEpisodeDataset,
)
from MixedEmbodiment_dual.dataloader_robot import RobotEpisodeDataset  # noqa: E402
from MixedEmbodiment_dual.dataloader_utils import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="MixedEmbodiment_dual ACT training (robot + human + mixed 3-cam)"
    )
    p.add_argument(
        "--sessions_root",
        type=str,
    ...
    p.add_argument("--mixed_robot_eef_dir", type=str, default=None)
    p.add_argument("--mixed_robot_side", choices=("left", "right"), default=None)
    p.add_argument("--mixed_hand_side", choices=("left", "right"), default=None)
    p.add_argument(
        "--robot_eef_dir",
        type=str,
        default=None,
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Same as --max_demos: use first N demos (sorted) per modality.",
    )
    p.add_argument(
        "--robot_first_n",
        type=int,
    ...
    p.add_argument(
        ...
        help=(
            f"Scale on mixed dual-route loss (default {DEFAULT_MIXED_LAMBDA}). "
            "L_mixed = mixed_lambda * 0.5 * (L_robot_route + L_human_route)."
        ),
    )
    p.add_argument(
        "--mixed_human_pose",
        type=str,
        choices=list(MIXED_HUMAN_POSE_MODES),
        default=MIXED_HUMAN_POSE_FULL8,
        help=(
            "Human-route pose on mixed batches: 'full8' uses all glued dims (default); "
            "'hand4' keeps only the hand half (zeros/masks the robot half)."
        ),
    )
    p.add_argument(
        "--reconstruction_loss",
    ...
    p.add_argument(
        ...
        default=True,
        help="Average available modality losses then one optimizer step (default). "
...
def resolve_demo_caps(cli: argparse.Namespace) -> argparse.Namespace:
    ...
    Priority per modality:
      --*_first_n  >  --first_n  >  --max_demos  >  None (all)
    """
    ...
    cli.mixed_demo_cap = cli.mixed_first_n if cli.mixed_first_n is not None else shared
    return cli
...
def apply_smoke_defaults(cli: argparse.Namespace) -> argparse.Namespace:
    ...
    cli.wandb = False
    if (
        cli.max_demos is None
        and cli.first_n is None
        and cli.robot_first_n is None
        and cli.human_first_n is None
        and getattr(cli, "mixed_first_n", None) is None
    ):
        cli.max_demos = 1
        cli.first_n = 1
    if cli.batch > 1:
...
class HomogeneousSubsetBatchSampler(Sampler[list[int]]):
    """
    Yield batches that stay inside one ConcatDataset child (e.g. all-human or all-mixed).

    Batch order is shuffled so human and mixed batches interleave; within each
    subset, indices are shuffled. Mixed is not over-sampled relative to human
    beyond appearing once per pass over the concat.
    """

    def __init__(self, subset_lengths: list[int], batch_size: int, *, shuffle: bool = True) -> None:
        if batch_size < 1:
            raise ValueError("batch_size must be >= 1")
        if not subset_lengths:
            raise ValueError("subset_lengths must be non-empty")
        self.subset_lengths = [int(n) for n in subset_lengths]
        self.batch_size = int(batch_size)
        self.shuffle = bool(shuffle)
        groups: list[list[int]] = []
        offset = 0
        for n in self.subset_lengths:
            groups.append(list(range(offset, offset + n)))
            offset += n
        self.groups = groups
    ...
    def __len__(self) -> int:
        return sum((n + self.batch_size - 1) // self.batch_size for n in self.subset_lengths)


def make_human_mixed_loader(
    human_ds,
    mixed_ds,
    batch_size: int,
    num_workers: int,
) -> DataLoader:
    """Human demos + mixed demos in one loader; each batch is pure human or pure mixed."""
    if mixed_ds is None:
        return make_loader(human_ds, batch_size, num_workers, shuffle=True)
    concat = ConcatDataset([human_ds, mixed_ds])
    sampler = HomogeneousSubsetBatchSampler(
        [len(human_ds), len(mixed_ds)],
        batch_size,
        shuffle=True,
    )
    return DataLoader(
...
def synthetic_robot_batch(batch_size: int, num_queries: int, h: int = 120, w: int = 160, device=None) -> dict:
    ...
    return {
        ...
        "joint_loss_mask": torch.ones(batch_size, ROBOT_JOINT_DIM, device=device),
...
def synthetic_human_batch(batch_size: int, num_queries: int, h: int = 120, w: int = 160, device=None) -> dict:
    ...
    return {
        ...
        "joint_loss_mask": torch.zeros(batch_size, ROBOT_JOINT_DIM, device=device),
        "has_joint_target": False,
        "is_pad": torch.zeros(batch_size, num_queries, dtype=torch.bool, device=device),
    }


def synthetic_mixed_batch(batch_size: int, num_queries: int, h: int = 120, w: int = 160, device=None) -> dict:
    device = device or torch.device("cpu")
...
def compute_batch_losses(
    ...
    *,
    ...
    hand_lambda,
    mixed_lambda,
    mixed_human_pose=None,
) -> dict:
    """
    Single-route losses:
      human: hand_lambda * (pose_recon + kl)
      robot: pose_recon + joint_recon + kl
    Mixed batches should use compute_mixed_dual_losses instead.
    """
    del mixed_lambda, mixed_human_pose  # unused on single-route path
    ...
    if embodiment == EMBODIMENT_MIXED:
        raise RuntimeError("Mixed batches must use compute_mixed_dual_losses (dual robot+human routes)")
    ...
    if embodiment == EMBODIMENT_ROBOT:
        name = "robot"
    elif embodiment == EMBODIMENT_HUMAN:
        name = "human"
    else:
        name = f"emb{embodiment}"
    return {
        "loss": loss,
...
def compute_mixed_dual_losses(
    ...
    *,
    ...
    hand_lambda,
    mixed_lambda,
    mixed_human_pose: str,
) -> dict:
    """
    Mixed Method A: robot route + human route, correct masks each.

    L = mixed_lambda * 0.5 * (L_robot_route + L_human_route)
    """
    del hand_lambda  # mixed dual gated by mixed_lambda only
    if int(batch["embodiment"]) != EMBODIMENT_MIXED:
        raise RuntimeError("compute_mixed_dual_losses expects a mixed batch")
    ...
    joint_actions = batch["joint_actions"].to(device)
    is_pad = batch["is_pad"].to(device)
    camera_mask_mixed = batch["camera_mask"].to(device)
    joint_dim_mask = batch.get("joint_loss_mask")
    if joint_dim_mask is None:
        raise RuntimeError("Mixed batch missing joint_loss_mask")
    joint_dim_mask = joint_dim_mask.to(device)
    bs = images.shape[0]

    # --- Robot route (mixed cam mask, joints, joint head) ---
    out_r = model(
        pose_state=pose_state,
        images=images,
        embodiment=EMBODIMENT_MIXED,
        joint_state=joint_state,
        camera_mask=camera_mask_mixed,
        pose_actions=pose_actions,
        joint_actions=joint_actions,
        has_joint_target=True,
        is_pad=is_pad,
    )
    if out_r["joint_pred"] is None:
        raise RuntimeError("Mixed robot route missing joint_pred")
    total_kld_r, *_ = kl_divergence(out_r["mu"], out_r["logvar"])
    kld_r = total_kld_r[0]
    pose_loss_r = masked_recon_loss(out_r["pose_pred"], pose_actions, is_pad, kind=recon_kind)
    joint_loss_r = masked_recon_loss(
        out_r["joint_pred"], joint_actions, is_pad, kind=recon_kind, dim_mask=joint_dim_mask
    )
    base_r = pose_w * pose_loss_r + joint_w * joint_loss_r + kl_w * kld_r

    # --- Human route (bird-only mask, human CVAE/proprio) ---
    pose_state_h, pose_actions_h, pose_dim_mask = apply_mixed_human_pose_mode(
        pose_state,
        pose_actions,
        joint_dim_mask,
        mode=mixed_human_pose,
    )
    camera_mask_human = torch.tensor(
        HUMAN_CAMERA_MASK, device=device, dtype=images.dtype
    ).view(1, NUM_CAMERAS).expand(bs, -1)
    joint_state_h = torch.zeros(bs, ROBOT_JOINT_DIM, device=device, dtype=joint_state.dtype)
    joint_actions_h = torch.zeros(
        bs, pose_actions.shape[1], ROBOT_JOINT_DIM, device=device, dtype=joint_actions.dtype
    )
    out_h = model(
        pose_state=pose_state_h,
        images=images,
        embodiment=EMBODIMENT_HUMAN,
        joint_state=joint_state_h,
        camera_mask=camera_mask_human,
        pose_actions=pose_actions_h,
        joint_actions=joint_actions_h,
        has_joint_target=False,
        is_pad=is_pad,
    )
    total_kld_h, *_ = kl_divergence(out_h["mu"], out_h["logvar"])
    kld_h = total_kld_h[0]
    pose_loss_h = masked_recon_loss(
        out_h["pose_pred"],
        pose_actions_h,
        is_pad,
        kind=recon_kind,
        dim_mask=pose_dim_mask,
    )
    base_h = pose_w * pose_loss_h + kl_w * kld_h

    loss = float(mixed_lambda) * 0.5 * (base_r + base_h)
    return {
        "loss": loss,
        "pose_loss": (0.5 * (pose_loss_r + pose_loss_h)).detach(),
        "joint_loss": joint_loss_r.detach(),
        "kld": (0.5 * (kld_r + kld_h)).detach(),
        "embodiment": "mixed",
        "has_joint_target": True,
        "pose_pred_shape": tuple(out_r["pose_pred"].shape),
        "joint_pred_shape": tuple(out_r["joint_pred"].shape),
        "mixed_robot_pose_loss": pose_loss_r.detach(),
        "mixed_human_pose_loss": pose_loss_h.detach(),
        "mixed_robot_joint_loss": joint_loss_r.detach(),
    }
...
def train_step_single(model, optimizer, scaler, batch, device, use_amp: bool, loss_cfg: dict) -> dict:
    ...
    with autocast(enabled=use_amp):
        if int(batch["embodiment"]) == EMBODIMENT_MIXED:
            stats = compute_mixed_dual_losses(model, batch, device, **loss_cfg)
        else:
    ...
    for k in ("mixed_robot_pose_loss", "mixed_human_pose_loss", "mixed_robot_joint_loss"):
        if k in stats:
            out[k] = float(stats[k].cpu())
    return out
...
def train_step_joint_modalities(
    ...
    """One optimizer step from mean loss over available modality batches."""
    if not batches:
    ...
    with autocast(enabled=use_amp):
        stats_list = []
        for b in batches:
            if int(b["embodiment"]) == EMBODIMENT_MIXED:
                stats_list.append(compute_mixed_dual_losses(model, b, device, **loss_cfg))
            else:
                stats_list.append(compute_batch_losses(model, b, device, **loss_cfg))
    ...
    for s in stats_list:
        if s["embodiment"] == "robot":
            ...
            if "mixed_human_pose_loss" in s:
                out["mixed_human_pose_loss"] = float(s["mixed_human_pose_loss"].cpu())
    return out
...
def main() -> None:
    ...
    if (
        cli.robot_demo_cap is not None
        or cli.human_demo_cap is not None
        or getattr(cli, "mixed_demo_cap", None) is not None
    ):
        print(
    ...
    # mixed_lambda / mixed_human_pose: dual-route bridge settings
    mixed_lambda = float(cli.mixed_lambda)
    mixed_human_pose = str(cli.mixed_human_pose)
    loss_cfg = {
        ...
        "mixed_human_pose": mixed_human_pose,
    }
    print(
        f"Mixed bridge: human_pose_mode={mixed_human_pose} mixed_lambda={mixed_lambda} "
        f"(mixed demos concat'd into human-side loader; dual robot+human routes)"
    )
    ...
    meta["mixed_human_pose"] = mixed_human_pose
    meta["mixed_bridge"] = (
        "human_mixed_concat_loader + dual robot/human routes on mixed batches"
    )
    if human_pose_dir is not None:
    ...
    if human_ds is None:
        human_mixed_loader = None
    else:
        human_mixed_loader = make_human_mixed_loader(human_ds, mixed_ds, cli.batch, cli.num_workers)
    steps_per_epoch = steps_per_epoch_from_loaders(robot_loader, human_mixed_loader)
    if cli.smoke:
    ...
    if human_mixed_loader is not None:
        meta["human_loader_batches"] = len(human_mixed_loader)
        meta["human_num_demos"] = int(human_ds.num_demos) if human_ds is not None else None
    if mixed_ds is not None:
        meta["mixed_num_demos"] = int(mixed_ds.num_demos)
        meta["mixed_in_human_loader"] = True
    else:
        meta["mixed_in_human_loader"] = False
    save_run_metadata(output_dir, meta)
    print(f"Run metadata -> {output_dir / 'run_metadata.json'}")

    if cli.dry_run:
        print("--- MixedEmbodiment_dual dry run ---")
        if human_mixed_loader is not None:
            batch = next(iter(human_mixed_loader))
            stats = train_step_single(model, optimizer, scaler, batch, device, use_amp, loss_cfg)
            print(f"Human/mixed-side step OK (embodiment={stats['embodiment']}): {stats}")
        else:
        ...
        if mixed_ds is not None:
            mixed_only_loader = make_loader(
                mixed_ds, min(cli.batch, max(1, int(mixed_ds.num_demos))), 0, shuffle=True
            )
            batch = next(iter(mixed_only_loader))
            stats = train_step_single(model, optimizer, scaler, batch, device, use_amp, loss_cfg)
            print(f"Mixed dual-route step OK: {stats}")
        else:
            batch = synthetic_mixed_batch(cli.batch, cli.num_queries, device=device)
            stats = train_step_single(model, optimizer, scaler, batch, device, use_amp, loss_cfg)
            print(f"Synthetic mixed dual-route step OK: {stats}")
    ...
    if robot_loader is None or human_mixed_loader is None:
        raise RuntimeError(
            "Full training requires both robot and human data "
            "(mixed is optional; folded into the human-side loader when present)."
        )
    ...
    best = float("inf")
    step = 0
    n_robot = len(robot_loader)
    n_hm = len(human_mixed_loader)
    print(
        f"Training schedule: {cli.epochs} epochs x {steps_per_epoch} steps/epoch "
        f"(= max(robot_batches={n_robot}, human_mixed_batches={n_hm}); "
        f"shorter side recycled; mixed not a separate recycled loader), "
        f"batch={cli.batch}, lr={cli.lr}, recon={cli.reconstruction_loss}, "
        f"kl_weight={cli.kl_weight}, hand_lambda={cli.hand_lambda}, "
        f"mixed_lambda={mixed_lambda}, mixed_human_pose={mixed_human_pose}, K={cli.num_queries}"
    )
    for epoch in range(cli.epochs):
        model.train()
        robot_iter = iter(robot_loader)
        hm_iter = iter(human_mixed_loader)
        running = {
            "robot_pose": 0.0,
            "robot_joint": 0.0,
            "human_pose": 0.0,
        ...
        for _ in loop:
            ...
            hm_batch, hm_iter = next_batch_recycling(hm_iter, human_mixed_loader)
            batches = [robot_batch, hm_batch]

            if cli.joint_modality_update:
                stats = train_step_joint_modalities(
                    model, optimizer, scaler, batches, device, use_amp, loss_cfg
                )
                step += 1
                ...
                running["mixed_human_pose"] += stats.get("mixed_human_pose_loss", 0.0)
                running["n_r"] += 1
                running["n_joint"] += 1
                if int(hm_batch["embodiment"]) == EMBODIMENT_HUMAN:
                    running["n_h"] += 1
                else:
                    running["n_m"] += 1
                if wandb_run is not None:
                    log = {
                        ...
                        "train/mixed_pose_loss": stats["mixed_pose_loss"],
                        "train/mixed_joint_loss": stats["mixed_joint_loss"],
                        "train/mixed_human_pose_loss": stats.get("mixed_human_pose_loss", 0.0),
