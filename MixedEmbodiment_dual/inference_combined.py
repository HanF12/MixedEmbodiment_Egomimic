...
but drives MixedEmbodiment_dual.MixedDETRVAE with:
  - embodiment = robot
  ...
  - MixedEmbodiment_dual training also has human/mixed modalities (full8 / hand4);
    this script uses the robot joint path only
...
Default checkpoint / norms point at mixed_dual_act_best (+ robot norms). For the
hand4-trained dual run, pass e.g.:
  --checkpoint mixed_dual_4hand_act_best.pth
  --normalization_path mixed_dual_4hand_normalization_stats_robot.npz
"""
...
DEFAULT_CHECKPOINT = "mixed_dual_act_best.pth"
DEFAULT_ROBOT_NORM = "mixed_dual_normalization_stats_robot.npz"
DEFAULT_HUMAN_NORM = "mixed_dual_normalization_stats_human.npz"
...
parser = argparse.ArgumentParser(
    description="MixedEmbodiment_dual ACT inference (robot/joint pathway; absolute joint targets)"
)
...
parser.add_argument("--max_joint_speed", type=float, default=0.4)
...
rospy.init_node("mixed_embodiment_dual_act_inference_robot", anonymous=True)
    ...
    while not rospy.is_shutdown():
        ...
        if bool(cli.display):
            ...
            if wall - last_preview_t >= min_dt:
                ...
                cv2.imshow("MixedEmbodiment_dual ACT Inference (robot)", preview)
