"""
MixedEmbodiment_dual ACT constants and helpers.
...
# Mixed dual-route: how the human path sees mixed pose targets.
#   full8 — use all 8 glued dims (hand slot + robot EEF slot)
#   hand4 — keep only the hand half; zero/mask the robot half
MIXED_HUMAN_POSE_FULL8 = "full8"
MIXED_HUMAN_POSE_HAND4 = "hand4"
...
def validate_camera_names(camera_names: list[str] | tuple[str, ...]) -> None:
    if tuple(camera_names) != MODEL_CAMERA_NAMES:
        raise ValueError(
            ...
            "MixedEmbodiment_dual uses fixed slots [bird, left_wrist, right_wrist]."
        )
...
def hand_pose_dim_mask_from_joint_loss_mask(joint_loss_mask: torch.Tensor) -> torch.Tensor:
    """
    Infer which [8] pose half is the human hand from mixed joint_loss_mask.

    Active left arm  -> hand is on the right half (dims 4:8)
    Active right arm -> hand is on the left half  (dims 0:4)
    """
    jlm = torch.as_tensor(joint_loss_mask, dtype=torch.float32)
    if jlm.ndim == 1:
        jlm = jlm.unsqueeze(0)
    if jlm.shape[-1] != ROBOT_JOINT_DIM:
        raise ValueError(f"joint_loss_mask last dim must be {ROBOT_JOINT_DIM}, got {tuple(jlm.shape)}")
    left_active = jlm[:, LEFT_ARM_SLICE].sum(dim=-1) > 0
    right_active = jlm[:, RIGHT_ARM_SLICE].sum(dim=-1) > 0
    if bool((left_active & right_active).any()) or bool((~left_active & ~right_active).any()):
        raise ValueError(
            "hand_pose_dim_mask_from_joint_loss_mask expects exactly one active arm per sample"
        )
    mask = torch.zeros(jlm.shape[0], POSE_DIM, dtype=torch.float32, device=jlm.device)
    # left robot => hand on right half
    mask[left_active, POSE_DIM_PER_SIDE:] = 1.0
    # right robot => hand on left half
    mask[right_active, :POSE_DIM_PER_SIDE] = 1.0
    return mask


def apply_mixed_human_pose_mode(
    pose_state: torch.Tensor,
    pose_actions: torch.Tensor,
    joint_loss_mask: torch.Tensor,
    *,
    mode: str,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor | None]:
    """
    Prepare pose tensors + optional dim mask for the human route on mixed batches.

    Returns (pose_state_h, pose_actions_h, pose_dim_mask_or_None).
    """
    mode = str(mode)
    if mode == MIXED_HUMAN_POSE_FULL8:
        return pose_state, pose_actions, None
    if mode != MIXED_HUMAN_POSE_HAND4:
        raise ValueError(f"Unknown mixed human pose mode {mode!r}; expected one of {MIXED_HUMAN_POSE_MODES}")
    hand_mask = hand_pose_dim_mask_from_joint_loss_mask(joint_loss_mask).to(
        device=pose_state.device, dtype=pose_state.dtype
    )  # [B, 8]
    pose_state_h = pose_state * hand_mask
    pose_actions_h = pose_actions * hand_mask.unsqueeze(1)
    return pose_state_h, pose_actions_h, hand_mask
...
def validate_run_metadata(metadata: dict[str, Any], *, num_queries: int | None = None) -> None:
    ...
    if tuple(metadata.get("model_camera_names", [])) != MODEL_CAMERA_NAMES:
        raise ValueError(
            "Saved model camera names do not match the fixed MixedEmbodiment_dual camera order"
        )
