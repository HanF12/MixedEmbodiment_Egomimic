"""Human hand episode dataset for Combined-relative-3cam ACT (bird + 6D xyz hand pose)."""
...
class HumanEpisodeDataset(Dataset):
    ...
    Hand pose from NPZ key `pose` with shape [T, 2, 10] -> xyz-only flattened [8]
    (rot6d dropped).
    ...
    def __init__(
        ...
        *,
        ...
        print("Loading human Combined-relative-3cam demos...")
        for csv_path in sync_csvs:
            ...
            if self.require_valid_pos:
                if valid_pos is None:
                    raise KeyError(f"{pose_by[rec_id].name} missing required validity key: valid_pos")
            ...
            for i in range(n_i):
                ...
                self.pose_data.append(flatten_bimanual_pose(pose_arr[pidx], rec_id=rec_id))  # [6]
        ...
        if self.num_demos == 0:
            raise FileNotFoundError("No complete human demos found for Combined-relative-3cam.")

        all_p = torch.stack(self.pose_data, dim=0)  # [N, 6]
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        pose_raw = self.pose_data[sample_idx]  # [6] absolute, chunk anchor
