...
- Pose = xyz only (6D); rot6d and gripper/open dropped at load
- Joint grippers treated as continuous
...
from Combined_relative_xyz_3cam.config import (  # noqa: E402
    DEFAULT_BATCH_SIZE,
    DEFAULT_HAND_LAMBDA,
...
from Combined_relative_xyz_3cam.core import build, kl_divergence  # noqa: E402
from Combined_relative_xyz_3cam.data_synchronization import (  # noqa: E402
    synchronize_human_hands,
    synchronize_robot_bimanual,
)
from Combined_relative_xyz_3cam.dataloader_human import HumanEpisodeDataset, collate_homogeneous  # noqa: E402
from Combined_relative_xyz_3cam.dataloader_robot import RobotEpisodeDataset  # noqa: E402
from Combined_relative_xyz_3cam.dataloader_utils import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Combined-relative-xyz-3cam mixed human+robot ACT training (relative pose actions)"
    )
    p.add_argument(
        "--robot_data_root",
        type=str,
...
def build_robot_sync_csvs(
    ...
    for demo_id in ids:
        ...
        for key in ("timestamps", "pose", "valid_pos"):
...
def build_human_sync_csvs(
    ...
    for demo_id in ids:
        ...
        if "valid_pos" not in pose_npz.files:
            raise KeyError(f"{pose[demo_id].name} missing required validity key 'valid_pos'")
        synchronize_human_hands(
            np.load(bird[demo_id]),
            np.load(front[demo_id]),
            pose_npz["timestamps"],
...
def main() -> None:
    ...
    if cli.dry_run:
        print("--- Combined-relative-xyz-3cam dry run ---")
        if human_loader is not None:
