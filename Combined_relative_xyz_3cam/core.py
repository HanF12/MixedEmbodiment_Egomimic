...
(see Combined_relative_xyz_3cam dataloaders). Joint head still predicts absolute joints.
...
Shared:
  ...
  - pose_action_head: H -> 6  (human primary + robot aux; xyz+gripper)

Not shared (modality routing = EgoMimic embodiment cue):
  - robot_input_proj: joints[14] -> H
  - human_input_proj: pose[6] -> H
...
class MixedDETRVAE(nn.Module):
    def __init__(self, backbones, transformer, encoder, num_queries, camera_names):
        ...
        self.human_input_proj = nn.Linear(POSE_DIM, hidden_dim)  # [B,6] -> [B,H]
        ...
        self.pose_action_head = nn.Linear(hidden_dim, POSE_DIM)  # [B,K,H] -> [B,K,6]
    ...
    def forward(
        ...
        *,
        ...
        """
        Returns dict:
          pose_pred [B,K,6], joint_pred [B,K,14] or None for human,
        ...
        if camera_mask is None:
            ...
            if camera_mask.shape != (bs, NUM_CAMERAS):
                raise ValueError(f"camera_mask must be [B,3], got {tuple(camera_mask.shape)}")
