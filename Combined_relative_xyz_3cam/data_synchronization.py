"""
Timestamp synchronization for Combined_relative_xyz_3cam training.
...
def xyz_only_valid_mask(
    *,
    ...
    """
    Per-frame mask: True iff required slots have valid xyz (position).

    Used by the xyz-only pose variant: open/gripper validity is ignored because
    gripper is not part of the pose prediction pathway.

    - ``required_slots`` defaults to both hands/arms ``(0, 1)``.
    - Orientation / ``valid_rot`` / ``valid_open`` are never consulted.
    - ``valid_pos`` is expected as [T, 2] (left=0, right=1).
    """
    if valid_pos is None:
        if n_frames is None:
            raise ValueError("Need valid_pos or n_frames")
        return np.ones((int(n_frames),), dtype=bool)

    a = np.asarray(valid_pos, dtype=bool)
    if a.ndim != 2 or a.shape[1] != 2:
        raise ValueError(f"valid_pos must have shape [T, 2], got {a.shape}")
    t = int(a.shape[0])
    ...
    return a[:, list(slots)].all(axis=1)
...
def xyz_gripper_valid_mask(
    *,
    ...
    """
    Legacy helper kept for compatibility. In this xyz-only package, prefer
    ``xyz_only_valid_mask`` (ignores ``valid_open``).
    """
    del valid_open  # gripper/open unused for pose pathway
    return xyz_only_valid_mask(
        valid_pos=valid_pos,
        n_frames=n_frames,
...
def full_hand_pose_valid_mask(
    *,
    ...
    """Deprecated alias: xyz-only validity (ignores rot/open)."""
    del valid_rot, valid_open
    return xyz_only_valid_mask(
        valid_pos=valid_pos,
        n_frames=n_frames,
        required_slots=required_slots,
...
def synchronize_robot_bimanual(
    ...
    *,
    ...
    xyz for both arms. Gripper/open and orientation are ignored (xyz-only pose).
    """
    del valid_rot, valid_open  # unused for xyz-only pose pathway
    ...
    if require_full_eef_pose and len(df) > 0:
        if valid_pos is None:
            raise ValueError(
                "require_full_eef_pose=True but missing NPZ mask valid_pos. "
                "Pass valid_pos from the EEF NPZ."
            )
        frame_ok = xyz_only_valid_mask(
            valid_pos=valid_pos,
        ...
        print(
            ...
            f"(dropped {n_before - len(df)}, require xyz for both arms; grip/orient ignored)"
        )
...
def synchronize_human_hands(
    ...
    *,
    ...
    valid xyz for both hands are dropped. Open/close and orientation are
    ignored (xyz-only pose pathway). Pass valid_pos from the NPZ.
    ...
    if require_full_pose and len(df) > 0:
        if valid_pos is None:
            raise ValueError(
                "require_full_pose=True but missing NPZ mask valid_pos. "
                "Pass valid_pos, or set require_full_pose=False."
            )
        frame_ok = xyz_only_valid_mask(
            valid_pos=valid_pos,
        ...
        print(
            ...
            f"(dropped {n_dropped}, require xyz for both hands; open/orient ignored)"
        )
