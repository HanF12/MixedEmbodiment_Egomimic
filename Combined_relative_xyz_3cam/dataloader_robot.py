"""Robot episode dataset for Combined-relative-3cam ACT (3 cameras, 6D xyz EEF pose + 14D joints)."""
...
from Combined_relative_xyz_3cam.config import (
    DEFAULT_NUM_QUERIES,
    EMBODIMENT_ROBOT,
    POSE_DIM,
    ROBOT_JOINT_DIM,
...
from Combined_relative_xyz_3cam.data_synchronization import xyz_only_valid_mask
from Combined_relative_xyz_3cam.dataloader_utils import (
...
class RobotEpisodeDataset(Dataset):
    ...
    Action-chunk convention:
      ...
      joint_actions[k] = joints[t+k]          (absolute)
    ...
    Returns dict (common batch schema):
      ...
      pose_state [8], pose_actions [K,8]  (xyz-only; rot dropped),
    ...
    def __init__(
        ...
        *,
        ...
        print("Loading robot Combined-relative-3cam demos...")
        for csv_path in sync_csvs:
            ...
            if self.require_valid_eef:
                if valid_pos is None:
                    raise KeyError(f"{eef_by[rec_id].name} missing required validity key: valid_pos")
            ...
            for i in range(n_i):
                ...
                eidx = int(df.loc[i, "eef_pose_index"])
                self.eef_pose_data.append(flatten_bimanual_pose(eef_arr[eidx], rec_id=rec_id))
        ...
        if self.num_demos == 0:
            raise FileNotFoundError("No complete robot demos found for Combined-relative-3cam.")

        all_q = torch.stack(self.joint_data, dim=0)  # [N, 14]
        all_e = torch.stack(self.eef_pose_data, dim=0)  # [N, 6]
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        pose_raw = self.eef_pose_data[sample_idx]  # [6] absolute, chunk anchor
        ...
        if len(pose_future_abs) != len(joint_future):
        ...
        pose_actions, is_pad = normalize_future_chunk(
            pose_future_rel, mean=self.eef_mean, std=self.eef_std, num_queries=self.num_queries
        )
