"""
Combined-relative-xyz (mixed human + robot) ACT constants and helpers.
...
Shared pose (human hands / robot EEF), EgoMimic-style shared head:
  6D = left 3 + right 3
  each side: xyz(3) only
  Source NPZ layout is still [2, 10] = xyz(3)+rot6d(6)+grip(1); rot and grip
  are dropped at load for the pose pathway.
...
def build_run_metadata(
    *,
    ...
    return {
        "variant": "combined-relative-xyz-only",
