"""
Mixed human + robot ACT training with relative pose actions + demo-level validation.

Copy of Combined_relative_3cam with held-out val demos per modality.
...
- Schedule: each epoch = one full pass over the longer modality's *train* loader
  (shorter modality recycled); then one val pass (no recycling)
...
from Combined_relative_3cam_val.config import (  # noqa: E402
    DEFAULT_BATCH_SIZE,
    DEFAULT_HAND_LAMBDA,
...
from Combined_relative_3cam_val.core import build, kl_divergence  # noqa: E402
from Combined_relative_3cam_val.data_synchronization import (  # noqa: E402
    synchronize_human_hands,
    synchronize_robot_bimanual,
)
from Combined_relative_3cam_val.dataloader_human import HumanEpisodeDataset, collate_homogeneous  # noqa: E402
from Combined_relative_3cam_val.dataloader_robot import RobotEpisodeDataset  # noqa: E402
from Combined_relative_3cam_val.dataloader_utils import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Combined-relative-3cam-val: mixed ACT + demo-level validation"
    )
    p.add_argument(
        "--robot_data_root",
        type=str,
    ...
    p.add_argument("--wandb_project", type=str, default="combined-relative-3cam-val")
...
def synthetic_robot_batch(batch_size: int, num_queries: int, h: int = 120, w: int = 160, device=None) -> dict:
    ...
    return {
        ...
        "images": torch.randn(batch_size, 3, 3, h, w, device=device),
        "camera_mask": torch.ones(batch_size, 3, device=device),
...
def split_demo_indices(n_demos: int, val_fraction: float, seed: int) -> tuple[list[int], list[int]]:
    """
    Demo-level train/val split.

    Returns (train_indices, val_indices). If validation is disabled or there are
    too few demos, val_indices is empty and all demos go to train.
    """
    n_demos = int(n_demos)
    if n_demos <= 0:
        return [], []
    ...
    if n_val >= n_demos:
        n_val = n_demos - 1
    val_ids = list(map(int, ids[:n_val]))
    train_ids = list(map(int, ids[n_val:]))
    return train_ids, val_ids


def split_episode_dataset(dataset, val_fraction: float, seed: int, *, label: str):
    """Wrap an episode dataset into train/val Subsets (or train-only)."""
    if dataset is None:
        return None, None, [], []
    n = len(dataset)
...
def compute_val_batch_losses(
    ...
    *,
    ...
    """
    Validation losses with prior latent (z=0): forward without action chunk / is_pad,
    then score predictions against held-out targets. No KL term (matches deploy).
    """
    pose_state = batch["pose_state"].to(device)
    joint_state = batch["joint_state"].to(device)
    images = batch["images"].to(device)
    ...
    if has_joint:
        if joint_pred is None:
            raise RuntimeError("Robot val batch missing joint_pred")
        joint_loss = masked_recon_loss(joint_pred, joint_actions, is_pad, kind=recon_kind)
        # Keep the same weighting as train recon terms (no KL on val).
        loss = pose_w * pose_loss + joint_w * joint_loss
    else:
        joint_loss = pose_pred.new_zeros(())
...
def run_validation(model, robot_val_loader, human_val_loader, device, use_amp: bool, loss_cfg: dict) -> dict:
    ...
    if robot_val_loader is not None:
        for batch in robot_val_loader:
            with autocast(enabled=use_amp):
                stats = compute_val_batch_losses(model, batch, device, **val_loss_cfg)
            sums["robot_pose"] += stats["pose_loss"]
            sums["robot_joint"] += stats["joint_loss"]
            sums["n_r"] += 1

    if human_val_loader is not None:
        for batch in human_val_loader:
            with autocast(enabled=use_amp):
                stats = compute_val_batch_losses(model, batch, device, **val_loss_cfg)
            sums["human_pose"] += stats["pose_loss"]
            sums["n_h"] += 1
...
def main() -> None:
    ...
    use_val = (not cli.no_val) and (float(cli.val_fraction) > 0.0) and (not cli.dry_run) and (not cli.smoke)
    ...
    robot_train_ds, robot_val_ds, robot_train_ids, robot_val_ids = split_episode_dataset(
        robot_ds, val_fraction, cli.val_seed, label="Robot"
    )
    human_train_ds, human_val_ds, human_train_ids, human_val_ids = split_episode_dataset(
        human_ds, val_fraction, cli.val_seed, label="Human"
    )
    # Fall back to full dataset if split produced no train subset (shouldn't happen).
    ...
    meta["val_seed"] = int(cli.val_seed)
    ...
    if cli.dry_run:
        print("--- Combined-relative-3cam-val dry run ---")
        if human_loader is not None:
    ...
    for epoch in range(cli.epochs):
        ...
        avg_train = (avg_rp + avg_rj + avg_hp) / 3.0
        print(
            f"Epoch {epoch+1}: train robot_pose={avg_rp:.6f} robot_joint={avg_rj:.6f} "
            f"human_pose={avg_hp:.6f} train_avg={avg_train:.6f}"
        )
        log_payload = {
            "epoch": epoch + 1,
        ...
        has_val = (robot_val_loader is not None and len(robot_val_loader) > 0) or (
            human_val_loader is not None and len(human_val_loader) > 0
        )
        if has_val:
            val_stats = run_validation(
                model, robot_val_loader, human_val_loader, device, use_amp, loss_cfg
            )
            def _fmt(v):
