"""
Combined-relative-3cam (mixed human + robot) ACT constants and helpers.

True 3-camera architecture — front camera is NOT a model slot.

Camera slot order is fixed as:
  [bird, left_wrist, right_wrist]  -> model cams cam0..cam2

Front RGB is still used only for timestamp sync CSVs (not fed to the network).
...
Robot joints:
  ...
  EEF pose grippers (NPZ) and joint grippers (NPY) are binarized at load with
  independent thresholds (ROBOT_NPZ_* / ROBOT_NPY_*).
...
# Multiply per-element recon error on gripper dims (pose + joints).
DEFAULT_GRIPPER_LOSS_WEIGHT = 5.0
# Independent binarize thresholds at load (human hand open/close left as-is).
ROBOT_NPZ_GRIPPER_BINARIZE_THRESHOLD = 0.8  # EEF pose from NPZ
ROBOT_NPY_GRIPPER_BINARIZE_THRESHOLD = 0.8  # joint grippers from NPY
# Back-compat alias
...
def build_run_metadata(
    *,
    ...
    return {
        "variant": "combined-relative-3cam-gripweight",
        ...
        "gripper_semantics": (
            ...
            f"robot EEF pose (NPZ) binarized at load with threshold "
            f"{float(npz_gripper_binarize_threshold)}; "
            f"robot joint grippers (NPY) binarized at load with threshold "
