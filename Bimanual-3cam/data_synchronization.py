...
def synchronize_bimanual_3cam(
    ...
    *,
    ...
    """Synchronize two joint streams and three camera streams into one CSV."""
    ...
    while all(idx < size for idx, size in zip((li, ri, lc, rc, bc), lengths)):
        pivot = max(
            left_joint_ts[li],
            right_joint_ts[ri],
            left_cam_ts[lc],
        ...
        if not all(idx < size for idx, size in zip((li, ri, lc, rc, bc), lengths)):
            break
    ...
    print(f"Synced {len(df)} quintuplets -> {out_path} (debug={debug})")
    return df
