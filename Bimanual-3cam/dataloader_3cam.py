...
class PreloadedBimanualEpisodeDataset(Dataset):
    """
    Preloads 3 RGB views plus 14D bimanual joint state.

    Camera order is fixed as:
      [left wrist, right wrist, bird]
    ...
    def __init__(
        ...
        *,
        ...
        print("Loading and synchronizing all bimanual-3cam recordings...")
        ...
        if self.num_demos == 0:
            raise FileNotFoundError(
                "No complete bimanual-3cam demonstrations found after matching sync CSVs to data files."
            )
        ...
        print(f"Finished initializing bimanual-3cam dataset with {self.num_demos} demos.")
    ...
    def __getitem__(self, idx: int):
        ...
        stacked_images = stack_camera_tensors(left_frame, right_frame, bird_frame)
...
def batch_sanity_check(train_loader: DataLoader) -> None:
    ...
    print("--- Bimanual-3cam batch sanity check ---")
    print(f"qpos shape:    {tuple(qpos.shape)}")
    print(f"image shape:   {tuple(image.shape)}  (expect cams=3)")
