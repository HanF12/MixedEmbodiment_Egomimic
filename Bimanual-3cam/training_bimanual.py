...
# Keep this package first so local modules are not shadowed by ALOHA-mimic.
...
from data_synchronization import synchronize_bimanual_3cam  # noqa: E402
from dataloader_3cam import (  # noqa: E402
    PreloadedBimanualEpisodeDataset,
    batch_sanity_check,
    demo_id_from_hash_filename,
    demo_id_from_joint_npy,
)
from core import build, kl_divergence  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bimanual-3cam ACT training script (no front camera)")
    ...
    parser.add_argument("--wandb_project", type=str, default="bimanual-3cam")
...
def build_sync_csvs(data_root: Path, sync_dir: Path, max_skew_s: float) -> None:
    ...
    if not shared_ids:
        raise FileNotFoundError(
            ...
            "Need left/right wrist, bird, and left/right joint timestamps."
        )

    sync_dir.mkdir(parents=True, exist_ok=True)
    for demo_id in shared_ids:
        out_csv = sync_dir / f"{demo_id}.csv"
        synchronize_bimanual_3cam(
...
def main() -> None:
    ...
    normalization_path = (
        ...
        else (output_dir / "normalization_stats_bimanual_3cam.npz")
    )
