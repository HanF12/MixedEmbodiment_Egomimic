...
- Gripper emphasis: --gripper_loss_weight scales recon error on pose grip dims
  (3,7) and joint grip dims (6,13); default 5.0 (this package only)
"""
...
from MixedEmbodiment_gripweight.build_sync import (  # noqa: E402
    _infer_preset,
    _resolve_eef_dir,
    _resolve_pose_dir,
    build_mixed_sync_csvs,
)
from MixedEmbodiment_gripweight.config import (  # noqa: E402
    DEFAULT_BATCH_SIZE,
...
from MixedEmbodiment_gripweight.core import build, kl_divergence  # noqa: E402
from MixedEmbodiment_gripweight.data_synchronization import (  # noqa: E402
    synchronize_human_hands,
    synchronize_robot_bimanual,
)
from MixedEmbodiment_gripweight.dataloader_human import HumanEpisodeDataset, collate_homogeneous  # noqa: E402
from MixedEmbodiment_gripweight.dataloader_mixed import (  # noqa: E402
    ConcatMixedEpisodeDataset,
    MixedEpisodeDataset,
)
from MixedEmbodiment_gripweight.dataloader_robot import RobotEpisodeDataset  # noqa: E402
from MixedEmbodiment_gripweight.dataloader_utils import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            "Scale per-element recon error on gripper dims in pose+joint actions "
            f"(pose indices {POSE_GRIP_INDICES}, joint indices {GRIPPER_INDICES}). "
...
def compute_batch_losses(
    ...
    *,
    ...
    pose_dim_w = torch.tensor(
        gripper_dim_weights(POSE_DIM, POSE_GRIP_INDICES, gripper_w),
        device=device,
        dtype=pose_pred.dtype,
    )
    pose_loss = masked_recon_loss(
        pose_pred, pose_actions, is_pad, kind=recon_kind, dim_weights=pose_dim_w
    )
    if has_joint:
        if joint_pred is None:
            raise RuntimeError("Robot/mixed batch missing joint_pred")
        joint_dim_w = torch.tensor(
            gripper_dim_weights(ROBOT_JOINT_DIM, GRIPPER_INDICES, gripper_w),
            device=device,
            dtype=joint_pred.dtype,
        )
        joint_loss = masked_recon_loss(
            ...
            is_pad,
            kind=recon_kind,
            dim_mask=joint_dim_mask,
            dim_weights=joint_dim_w,
        )
...
def main() -> None:
    ...
    # mixed_lambda resolved after loaders exist (may be auto from recycle ratio).
