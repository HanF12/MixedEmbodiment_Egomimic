#!/usr/bin/env python
"""
Combined-relative mixed-ACT inference for *no-pose-proprio* checkpoints
(robot / joint pathway only).

Same I/O as inference_combined.py, but loads policies trained without human
pose state inputs. Those checkpoints replace:
  human_input_proj       -> human_input_const [H]
  human_cvae_state_proj  -> human_cvae_state_const [H]
Robot joint proprio / joint head are unchanged. Example:
  mixed_act_epoch_9000_omar_2.pth
"""
...
DEFAULT_CHECKPOINT = "mixed_act_epoch_9000_omar_2.pth"
DEFAULT_ROBOT_NORM = "normalization_stats_robot_omar_2.npz"
DEFAULT_HUMAN_NORM = "normalization_stats_human_3cam.npz"


def convert_model_to_no_pose_proprio(model: nn.Module) -> nn.Module:
    """
    Match Omar-style MixedDETRVAE weights: human proprio / CVAE-state are
    learned constants, not Linear(pose[8] -> H). Robot path unchanged.
    """
    ...
    if not hasattr(model, "human_input_const"):
        model.register_parameter(
            "human_input_const",
            nn.Parameter(torch.zeros(hidden_dim, device=device, dtype=dtype)),
        )
...
def install_no_pose_proprio_callables(model: nn.Module) -> nn.Module:
    """Restore human_*_proj call sites so core.forward / _encode_latent keep working."""

    def human_input_proj(pose_state: torch.Tensor) -> torch.Tensor:
        bs = pose_state.shape[0]
        return model.human_input_const.unsqueeze(0).expand(bs, -1)

    def human_cvae_state_proj(pose_state: torch.Tensor) -> torch.Tensor:
        bs = pose_state.shape[0]
        return model.human_cvae_state_const.unsqueeze(0).expand(bs, -1)
...
parser = argparse.ArgumentParser(
    description=(
        "Combined-relative ACT inference for no-pose-proprio checkpoints "
        "(robot/joint pathway; absolute joint targets)"
    )
)
...
model.eval()
print(f"Loaded no-pose-proprio checkpoint: {checkpoint_path}")
print("  human_input_const / human_cvae_state_const enabled (pose proprio unused)")
