#!/usr/bin/env python
"""
Combined-relative mixed-ACT inference for checkpoints with *no human proprio*
(robot / joint pathway only).

Same I/O as inference_combined.py, but loads policies that removed human state
adapters entirely (neither pose Linear projs nor Omar-style constants):
  - no human_input_proj / human_input_const
  - no human_cvae_state_proj / human_cvae_state_const
Still keeps human_cvae_action_proj + pose_action_head. Robot joint path unchanged.

Example checkpoints: mixed_act_epoch_9000_42.pth, mixed_act_epoch_9000_omar_3.pth
"""
...
DEFAULT_CHECKPOINT = "mixed_act_epoch_9000_42.pth"
DEFAULT_ROBOT_NORM = "normalization_stats_robot_42.npz"
...
parser = argparse.ArgumentParser(
    description=(
        "Combined-relative ACT inference for no-human-proprio checkpoints "
...
print("  stripped human_input_* / human_cvae_state_* (proj and const)")
...
rospy.init_node("combined_relative_act_inference_no_human_proprio", anonymous=True)
